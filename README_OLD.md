...
- **Brownfield-first**: works great beyond 0�?. TestSpec separates the source of truth from proposals: `TestSpec/specs/` (current truth) and `TestSpec/changes/` (proposed updates). This keeps diffs explicit and manageable across features.
- **Change tracking**: proposals, tasks, and spec deltas live together; archiving merges the approved updates back into specs.
- **Compared to spec-kit & Kiro**: those shine for brand-new features (0�?). TestSpec also excels when modifying existing behavior (1→n), especially when updates span multiple specs.
...
| **CodeBuddy Code (CLI)** | `/TestSpec:proposal`, `/TestSpec:apply`, `/TestSpec:archive` (`.codebuddy/commands/`) �?see [docs](https://www.codebuddy.ai/cli) |
...
| **CoStrict** | `/TestSpec-proposal`, `/TestSpec-apply`, `/TestSpec-archive` (`.cospec/TestSpec/commands/`) �?see [docs](https://costrict.ai)|
...
| **Qoder** | `/TestSpec:proposal`, `/TestSpec:apply`, `/TestSpec:archive` (`.qoder/commands/TestSpec/`) �?see [docs](https://qoder.com) |
...
### vs. spec-kit
TestSpec’s two-folder model (`TestSpec/specs/` for the current truth, `TestSpec/changes/` for proposed updates) keeps state and diffs separate. This scales when you modify existing features or touch multiple specs. spec-kit is strong for greenfield/0�? but provides less structure for cross-spec updates and evolving features.
...
1. **Initialize TestSpec** �?Run `TestSpec init` in your repo.
...
<details>
<summary><strong>Telemetry</strong> �?TestSpec collects anonymous usage stats (opt-out: <code>TestSpec_TELEMETRY=0</code>)</summary>
