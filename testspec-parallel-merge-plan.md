# TestSpec Parallel Delta Remediation Plan
...
- The tooling cannot detect divergence between the change author鈥檚 starting point and the live spec, so parallel development corrupts the source of truth without warning.
...
- There is no warning, diff, or conflict indicator鈥攖he archive completes successfully, and the source-of-truth spec now omits a shipped scenario.
...
- Detect and block speculative archives when the live spec diverges from the author鈥檚 base.
...
### Phase 0 鈥?Stop the Bleeding (Detection & Guardrails)
...
4. **Document interim manual mitigation.**
    - Update `testspec/AGENTS.md` and docs so contributors know to rerun `testspec change sync` (see Phase 1) whenever another change lands.

_Outcome:_ We prevent data loss immediately while we work on a richer merge story.

### Phase 1 鈥?Add a Rebase Workflow (Author-Side Merge)
1. **Introduce `testspec change sync <id>` (or `rebase`).**
   - Reads the stored base snapshot, the current spec, and the author鈥檚 delta.
...
2. **Enrich validator messages.**
    - `testspec validate` should flag unresolved conflict markers or fingerprint mismatches so errors appear early in the workflow.
...
### Phase 2 鈥?Increase Delta Granularity
...
### Phase 3 鈥?Structured Spec Graph (Long-Term)
...
3. **Integrate with Git directly.**
   - Offer optional `testspec branch` scaffolding that aligns spec changes with Git branches, letting teams leverage Git鈥檚 conflict editor for the markdown IR.

_Outcome:_ TestSpec graduates from replace-based updates to a resilient, intent-preserving spec management platform.
...
1. Prototype fingerprint capture during `testspec change validate` and block archive on mismatches.
2. Ship `testspec change sync` with line-based diff3 merging and conflict markers.
