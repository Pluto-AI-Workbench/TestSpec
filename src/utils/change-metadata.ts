...
const METADATA_FILENAME = '.testspec.yaml';
...
/**
 * Writes change metadata to .testspec.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .testspec.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .testspec.yaml metadata (if exists)
 * 3. Schema from testspec/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/testspec/changes/change-name)
