/**
 * Legacy cleanup module for detecting and removing TestSpec artifacts
...
import { TESTSPEC_MARKERS } from './config.js';

/**
 * Legacy config file names from the old ToolRegistry.
 * These were config files created at project root with TestSpec markers.
 */
export const LEGACY_CONFIG_FILES = [
  ...
  'QODER.md',
  'IFLOW.md',
  'AGENTS.md', // root AGENTS.md (not testspec/AGENTS.md)
  'QWEN.md',
] as const;
...
export const LEGACY_SLASH_COMMAND_PATHS: Record<string, LegacySlashCommandPattern> = {
  // Directory-based: .tooldir/commands/testspec/ or .tooldir/commands/testspec/*.md
  'claude': { type: 'directory', path: '.claude/commands/testspec' },
  'codebuddy': { type: 'directory', path: '.codebuddy/commands/testspec' },
  'qoder': { type: 'directory', path: '.qoder/commands/testspec' },
  'lingma': { type: 'directory', path: '.lingma/commands/testspec' },
  'crush': { type: 'directory', path: '.crush/commands/testspec' },
  'gemini': { type: 'directory', path: '.gemini/commands/testspec' },
  'costrict': { type: 'directory', path: '.cospec/testspec/commands' },

  // File-based: individual testspec-*.md files in a commands/workflows/prompts folder
  'cursor': { type: 'files', pattern: '.cursor/commands/testspec-*.md' },
  'windsurf': { type: 'files', pattern: '.windsurf/workflows/testspec-*.md' },
  'kilocode': { type: 'files', pattern: '.kilocode/workflows/testspec-*.md' },
  'kiro': { type: 'files', pattern: '.kiro/prompts/testspec-*.prompt.md' },
  'github-copilot': { type: 'files', pattern: '.github/prompts/testspec-*.prompt.md' },
  'amazon-q': { type: 'files', pattern: '.amazonq/prompts/testspec-*.md' },
  'cline': { type: 'files', pattern: '.clinerules/workflows/testspec-*.md' },
  'roocode': { type: 'files', pattern: '.roo/commands/testspec-*.md' },
  'auggie': { type: 'files', pattern: '.augment/commands/testspec-*.md' },
  'factory': { type: 'files', pattern: '.factory/commands/testspec-*.md' },
  'opencode': { type: 'files', pattern: ['.opencode/command/testspec-*.md'] },
  'continue': { type: 'files', pattern: '.continue/prompts/testspec-*.prompt' },
  'antigravity': { type: 'files', pattern: '.agent/workflows/testspec-*.md' },
  'iflow': { type: 'files', pattern: '.iflow/commands/testspec-*.md' },
  'junie': { type: 'files', pattern: ['.junie/commands/testspec-*.md'] },
  'qwen': { type: 'files', pattern: '.qwen/commands/testspec-*.toml' },
  'codex': { type: 'files', pattern: '.codex/prompts/testspec-*.md' },
};
...
export interface LegacyDetectionResult {
  /** Config files with TestSpec markers detected */
  configFiles: string[];
  ...
  /** Whether testspec/AGENTS.md exists */
  hasOpenspecAgents: boolean;
  /** Whether testspec/project.md exists (preserved, migration hint only) */
  hasProjectMd: boolean;
  /** Whether root AGENTS.md has TestSpec markers */
...
/**
 * Detects all legacy TestSpec artifacts in a project.
 *
...
/**
 * Detects legacy config files with TestSpec markers.
 ...
 *
 * @param projectPath - The root path of the project
 * @param pattern - Glob pattern like '.cursor/commands/testspec-*.md'
...
async function findLegacySlashCommandFiles(
  ...
  try {
    ...
    // testspec-*.prompt.md -> /^testspec-.*\.prompt\.md$/
    // testspec-*.toml -> /^testspec-.*\.toml$/
...
/**
 * Detects legacy TestSpec structure files (AGENTS.md and project.md).
 *
 * @param projectPath - The root path of the project
...
export async function detectLegacyStructureFiles(
  ...
  // Check for testspec/AGENTS.md
  const testspecAgentsPath = FileSystemUtils.joinPath(projectPath, 'testspec', 'AGENTS.md');
  hasOpenspecAgents = await FileSystemUtils.fileExists(testspecAgentsPath);

  // Check for testspec/project.md (for migration messaging, not deleted)
  const projectMdPath = FileSystemUtils.joinPath(projectPath, 'testspec', 'project.md');
  hasProjectMd = await FileSystemUtils.fileExists(projectMdPath);

  // Check for root AGENTS.md with TestSpec markers
  ...
  if (await FileSystemUtils.fileExists(rootAgentsPath)) {
    ...
    hasRootAgentsWithMarkers = hasTestSpecMarkers(content);
  }
...
/**
 * Checks if content contains TestSpec markers.
 *
 * @param content - File content to check
 * @returns True if both start and end markers are present
 */
export function hasTestSpecMarkers(content: string): boolean {
  return (
    content.includes(TESTSPEC_MARKERS.start) && content.includes(TESTSPEC_MARKERS.end)
  );
}

/**
 * Checks if file content is 100% TestSpec content (only markers and whitespace outside).
 *
 * @param content - File content to check
 * @returns True if content outside markers is only whitespace
 */
export function isOnlyTestSpecContent(content: string): boolean {
  const startIndex = content.indexOf(TESTSPEC_MARKERS.start);
  const endIndex = content.indexOf(TESTSPEC_MARKERS.end);
  ...
  const after = content.substring(endIndex + TESTSPEC_MARKERS.end.length);

  return before.trim() === '' && after.trim() === '';
}

/**
 * Removes the TestSpec marker block from file content.
 ...
 *
 * @param content - File content with TestSpec markers
 * @returns Content with marker block removed
 */
export function removeMarkerBlock(content: string): string {
  return removeMarkerBlockUtil(content, TESTSPEC_MARKERS.start, TESTSPEC_MARKERS.end);
}
...
/**
 * Cleans up legacy TestSpec artifacts from a project.
 * Preserves testspec/project.md (shows migration hint instead of deleting).
 *
 * @param projectPath - The root path of the project
...
export async function cleanupLegacyArtifacts(
  ...
  // Delete legacy slash command directories (these are 100% TestSpec-managed)
  ...
  // Delete legacy slash command files (these are 100% TestSpec-managed)
  ...
  // Delete testspec/AGENTS.md (this is inside testspec/, it's TestSpec-managed)
  if (detection.hasOpenspecAgents) {
    const agentsPath = FileSystemUtils.joinPath(projectPath, 'testspec', 'AGENTS.md');
    if (await FileSystemUtils.fileExists(agentsPath)) {
      try {
        await fs.unlink(agentsPath);
        result.deletedFiles.push('testspec/AGENTS.md');
      } catch (error: any) {
        result.errors.push(`Failed to delete testspec/AGENTS.md: ${error.message}`);
      }
    }
  }

  // Handle root AGENTS.md with TestSpec markers - remove markers only, NEVER delete
...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const dir of result.deletedDirs) {
      lines.push(`  ✓ Removed ${dir}/ (replaced by /testspec:*)`);
    }

    for (const file of result.modifiedFiles) {
      lines.push(`  ✓ Removed TestSpec markers from ${file}`);
    }
  }
 ...
 * Only includes TestSpec-managed files (slash commands, testspec/AGENTS.md).
...
function buildRemovalsList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  // Slash command directories (these are 100% TestSpec-managed)
  ...
  // Slash command files (these are 100% TestSpec-managed)
  for (const file of detection.slashCommandFiles) {
    removals.push({ path: file, explanation: 'replaced by skills/' });
  }

  // testspec/AGENTS.md (inside testspec/, it's TestSpec-managed)
  if (detection.hasOpenspecAgents) {
    removals.push({ path: 'testspec/AGENTS.md', explanation: 'obsolete workflow file' });
  }
...
function buildUpdatesList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  for (const file of detection.configFilesToUpdate) {
    updates.push({ path: file, explanation: 'removing TestSpec markers' });
  }
...
export function formatDetectionSummary(detection: LegacyDetectionResult): string {
  ...
  lines.push(chalk.bold('Upgrading to the new TestSpec'));
  lines.push('');
  lines.push('TestSpec now uses agent skills, the emerging standard across coding');
  ...
  if (updates.length > 0) {
    ...
    lines.push(chalk.dim('TestSpec markers will be removed, your content preserved:'));
  ...
  for (const file of detection.slashCommandFiles) {
    ...
    for (const [toolId, pattern] of Object.entries(LEGACY_SLASH_COMMAND_PATHS)) {
      if (pattern.type === 'files' && pattern.pattern) {
        ...
        // e.g., '.cursor/commands/testspec-*.md' -> /^\.cursor\/commands\/testspec-.*\.md$/
        const patterns = Array.isArray(pattern.pattern) ? pattern.pattern : [pattern.pattern];
        let matched = false;
        for (const p of patterns) {
...
export function formatProjectMdMigrationHint(): string {
  ...
  lines.push('  - testspec/project.md');
  lines.push(chalk.dim('    We won\'t delete this file. It may contain useful project context.'));
  lines.push('');
  lines.push(chalk.dim('    The new testspec/config.yaml has a "context:" section for planning'));
  lines.push(chalk.dim('    context. This is included in every TestSpec request and works more'));
