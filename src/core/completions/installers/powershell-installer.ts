...
export class PowerShellInstaller {
  ...
  getInstallationPath(): string {
    ...
    return path.join(profileDir, 'TestSpecCompletion.ps1');
  }
  ...
  async configureProfile(scriptPath: string): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Add TestSpec completion configuration with markers
        const testspecBlock = [
          '',
        ...
        const newContent = profileContent + testspecBlock;
  ...
  async removeProfileConfig(): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Remove TESTSPEC:START -> TESTSPEC:END block
        const startMarker = '# TESTSPEC:START';
        const endMarker = '# TESTSPEC:END';
        const startIndex = profileContent.indexOf(startMarker);

        if (startIndex === -1) {
          continue; // No TestSpec block found
        }
