...
/**
 * Generates Bash completion scripts for the TestSpec CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for TestSpec CLI
...
${helpers}
complete -F _testspec_completion testspec
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_testspec_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_testspec_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_testspec_complete_items`);
        break;
      case 'schema-name':
