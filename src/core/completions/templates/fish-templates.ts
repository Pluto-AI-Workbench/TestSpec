...
function __fish_testspec_using_subcommand
...
function __fish_testspec_no_subcommand
    set -l cmd (commandline -opc)
    test (count $cmd) -eq 1
end`;
...
function __fish_testspec_changes
    testspec __complete changes 2>/dev/null | while read -l id desc
        printf '%s\\t%s\\n' "$id" "$desc"
    end
end

function __fish_testspec_specs
    testspec __complete specs 2>/dev/null | while read -l id desc
        printf '%s\\t%s\\n' "$id" "$desc"
    end
end
