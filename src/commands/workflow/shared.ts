...
/**
 * Returns the list of available change directory names under testspec/changes/.
...
export async function getAvailableChanges(projectRoot: string): Promise<string[]> {
  const changesPath = path.join(projectRoot, 'testspec', 'changes');
  try {
...
export async function validateChangeExists(
  ...
  if (!changeName) {
    ...
    if (available.length === 0) {
      throw new Error('No changes found. Create one with: testspec new change <name>');
    }
    throw new Error(
  ...
  const changePath = path.join(projectRoot, 'testspec', 'changes', changeName);
  ...
  if (!exists) {
    ...
    if (available.length === 0) {
      throw new Error(
        `Change '${changeName}' not found. No changes exist. Create one with: testspec new change <name>`
      );
    }
    throw new Error(
