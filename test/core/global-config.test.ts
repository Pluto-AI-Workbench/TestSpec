...
describe('global-config', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `testspec-global-config-test-${Date.now()}`);
  ...
  describe('constants', () => {
    it('should export correct directory name', () => {
      expect(GLOBAL_CONFIG_DIR_NAME).toBe('testspec');
    });
  ...
  describe('getGlobalConfigDir', () => {
    it('should use XDG_CONFIG_HOME when set', () => {
      ...
      expect(result).toBe(path.join(tempDir, 'testspec'));
    });
    ...
    it('should fall back to ~/.config on Unix/macOS without XDG_CONFIG_HOME', () => {
      ...
      // On non-Windows, should use ~/.config/testspec
      if (os.platform() !== 'win32') {
        expect(result).toBe(path.join(os.homedir(), '.config', 'testspec'));
      }
    });
    ...
    it('should use APPDATA on Windows when XDG_CONFIG_HOME is not set', () => {
      ...
      if (os.platform() === 'win32') {
        ...
        if (appData) {
          const result = getGlobalConfigDir();
          expect(result).toBe(path.join(appData, 'testspec'));
        }
      }
    });
  });
  ...
  describe('getGlobalConfigPath', () => {
    it('should return path to config.json in config directory', () => {
      ...
      expect(result).toBe(path.join(tempDir, 'testspec', 'config.json'));
    });
  });
  ...
  describe('getGlobalConfig', () => {
    ...
    it('should not create directory when reading non-existent config', () => {
      process.env.XDG_CONFIG_HOME = tempDir;
      const configDir = path.join(tempDir, 'testspec');
  ...
  describe('saveGlobalConfig', () => {
    ...
    it('should write config to file', () => {
      process.env.XDG_CONFIG_HOME = tempDir;
      const configPath = path.join(tempDir, 'testspec', 'config.json');
