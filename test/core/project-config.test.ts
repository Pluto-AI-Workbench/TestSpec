...
describe('project-config', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'testspec-test-config-'));
...
`
      ...
      it('should handle completely invalid YAML gracefully', () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          expect.stringContaining('Failed to parse testspec/config.yaml'),
          expect.anything()
        );
      });
...
`
    ...
    describe('.yml/.yaml precedence', () => {
      ...
      it('should return null when testspec directory does not exist', () => {
        const config = readProjectConfig(tempDir);
  ...
  describe('suggestSchemas', () => {
    ...
    it('should include fix instruction', () => {
      ...
      expect(message).toContain(
        "Fix: Edit testspec/config.yaml and change 'schema: wrong-schema' to a valid schema name"
      );
    });
