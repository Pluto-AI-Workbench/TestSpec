...
import { TESTSPEC_MARKERS } from "../../src/core/config.js";
...
describe("legacy-cleanup", () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `testspec-legacy-test-${randomUUID()}`);
    await fs.mkdir(testDir, { recursive: true });
    // Create testspec directory structure
  ...
  describe("hasTestSpecMarkers", () => {
    it("should return true when both markers are present", () => {
      const content = `Some content
${TESTSPEC_MARKERS.start}
TestSpec content
${TESTSPEC_MARKERS.end}
More content`;
      expect(hasTestSpecMarkers(content)).toBe(true);
    });
...
TestSpec content
${TESTSPEC_MARKERS.end}`;
      expect(hasTestSpecMarkers(content)).toBe(false);
    });

    it("should return false when end marker is missing", () => {
      const content = `${TESTSPEC_MARKERS.start}
TestSpec content
Some content`;
      expect(hasTestSpecMarkers(content)).toBe(false);
    });
  ...
  describe("isOnlyTestSpecContent", () => {
    it("should return true when content is only markers and whitespace outside", () => {
      const content = `${TESTSPEC_MARKERS.start}
TestSpec content here
${TESTSPEC_MARKERS.end}`;
      expect(isOnlyTestSpecContent(content)).toBe(true);
    });
...
TestSpec content
${TESTSPEC_MARKERS.end}`;
      expect(isOnlyTestSpecContent(content)).toBe(false);
    });
...
User content here`;
    ...
    it("should return false when end marker comes before start marker", () => {
      const content = `${TESTSPEC_MARKERS.end}
Content
${TESTSPEC_MARKERS.start}`;
      expect(isOnlyTestSpecContent(content)).toBe(false);
    });
...
${TESTSPEC_MARKERS.end}`;
      ...
      expect(result).not.toContain(TESTSPEC_MARKERS.start);
      expect(result).not.toContain(TESTSPEC_MARKERS.end);
    });
...
${TESTSPEC_MARKERS.start}`;
      const result = removeMarkerBlock(content);
      expect(result).toContain(TESTSPEC_MARKERS.end);
      expect(result).toContain(TESTSPEC_MARKERS.start);
    });

    it("should ignore inline mentions of markers and only remove actual block", () => {
      const content = `Intro referencing ${TESTSPEC_MARKERS.start} and ${TESTSPEC_MARKERS.end} inline.
...
After content`;
  ...
  describe("detectLegacyConfigFiles", () => {
    it("should detect CLAUDE.md with TestSpec markers and put in update list", async () => {
...
${TESTSPEC_MARKERS.end}`,
    ...
    it("should not detect files without TestSpec markers", async () => {
      const claudePath = path.join(testDir, "CLAUDE.md");
  ...
  describe("detectLegacySlashCommands", () => {
    it("should detect legacy Claude slash command directory", async () => {
      const dirPath = path.join(testDir, ".claude", "commands", "testspec");
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, "proposal.md"), "content");

      const result = await detectLegacySlashCommands(testDir);
      expect(result.directories).toContain(".claude/commands/testspec");
    });

    it("should detect legacy Cursor slash command files", async () => {
      const dirPath = path.join(testDir, ".cursor", "commands");
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, "testspec-proposal.md"), "content");
      await fs.writeFile(path.join(dirPath, "testspec-apply.md"), "content");

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain(".cursor/commands/testspec-proposal.md");
      expect(result.files).toContain(".cursor/commands/testspec-apply.md");
    });

    it("should detect legacy Windsurf workflow files", async () => {
      const dirPath = path.join(testDir, ".windsurf", "workflows");
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, "testspec-archive.md"), "content");

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain(".windsurf/workflows/testspec-archive.md");
    });

    it("should detect multiple tool directories and files", async () => {
      // Create directory-based
      await fs.mkdir(path.join(testDir, ".claude", "commands", "testspec"), {
        recursive: true,
      });
      await fs.mkdir(path.join(testDir, ".qoder", "commands", "testspec"), {
        recursive: true,
      });
      ...
      await fs.writeFile(
        path.join(testDir, ".cursor", "commands", "testspec-proposal.md"),
        "content",
      );

      const result = await detectLegacySlashCommands(testDir);
      expect(result.directories).toContain(".claude/commands/testspec");
      expect(result.directories).toContain(".qoder/commands/testspec");
      expect(result.files).toContain(".cursor/commands/testspec-proposal.md");
    });

    it("should not detect non-testspec files", async () => {
      const dirPath = path.join(testDir, ".cursor", "commands");
      await fs.mkdir(dirPath, { recursive: true });
    ...
    it("should detect TOML-based slash commands for Qwen", async () => {
      ...
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(
        path.join(dirPath, "testspec-proposal.toml"),
        "content",
      );

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain(".qwen/commands/testspec-proposal.toml");
    });
    ...
    it("should detect Continue prompt files", async () => {
      ...
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(
        path.join(dirPath, "testspec-apply.prompt"),
        "content",
      );

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain(".continue/prompts/testspec-apply.prompt");
    });

    it("should detect legacy OpenCode testspec-* command files", async () => {
      const dirPath = path.join(testDir, ".opencode", "command");
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, "testspec-propose.md"), "content");

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain(".opencode/command/testspec-propose.md");
    });

    it("should detect legacy OpenCode testspec-* command files", async () => {
      const dirPath = path.join(testDir, ".opencode", "command");
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, "testspec-new.md"), "content");

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain(".opencode/command/testspec-new.md");
    });

    it("should detect both testspec-* and testspec-* OpenCode command files", async () => {
      const dirPath = path.join(testDir, ".opencode", "command");
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, "testspec-propose.md"), "content");
      await fs.writeFile(path.join(dirPath, "testspec-new.md"), "content");
  ...
  describe("detectLegacyStructureFiles", () => {
    it("should detect testspec/AGENTS.md", async () => {
      const agentsPath = path.join(testDir, "testspec", "AGENTS.md");
    ...
    it("should detect testspec/project.md", async () => {
      const projectPath = path.join(testDir, "testspec", "project.md");
    ...
    it("should detect root AGENTS.md with TestSpec markers", async () => {
...
${TESTSPEC_MARKERS.end}`,
  ...
  describe("detectLegacyArtifacts", () => {
    ...
    it("should return hasLegacyArtifacts: true when slash commands are found", async () => {
      ...
      const result = await detectLegacyArtifacts(testDir);
      expect(result.hasLegacyArtifacts).toBe(true);
      expect(result.slashCommandDirs).toContain(".claude/commands/testspec");
    });

    it("should return hasLegacyArtifacts: true when testspec/AGENTS.md is found", async () => {
      await fs.writeFile(
        path.join(testDir, "testspec", "AGENTS.md"),
        "content",
      );
    ...
    it("should detect project.md for migration hint (it is preserved, not deleted)", async () => {
      await fs.writeFile(
        path.join(testDir, "testspec", "project.md"),
        "content",
      );
  ...
  describe("cleanupLegacyArtifacts", () => {
    it("should remove markers from config files that have only TestSpec content (never delete)", async () => {
      const claudePath = path.join(testDir, "CLAUDE.md");
      await fs.writeFile(
        claudePath,
        `${TESTSPEC_MARKERS.start}\nContent\n${TESTSPEC_MARKERS.end}`,
      ...
      expect(content).not.toContain(TESTSPEC_MARKERS.start);
      expect(content).not.toContain(TESTSPEC_MARKERS.end);
    });
...
${TESTSPEC_MARKERS.end}`,
    ...
    it("should delete legacy slash command directories", async () => {
      ...
      expect(result.deletedDirs).toContain(".claude/commands/testspec");
    ...
    it("should delete legacy slash command files", async () => {
      const dirPath = path.join(testDir, ".cursor", "commands");
      await fs.mkdir(dirPath, { recursive: true });
      const filePath = path.join(dirPath, "testspec-proposal.md");
      ...
      expect(result.deletedFiles).toContain(
        ".cursor/commands/testspec-proposal.md",
      );
      await expect(fs.access(filePath)).rejects.toThrow();
    });

    it("should delete testspec/AGENTS.md", async () => {
      const agentsPath = path.join(testDir, "testspec", "AGENTS.md");
      ...
      expect(result.deletedFiles).toContain("testspec/AGENTS.md");
      await expect(fs.access(agentsPath)).rejects.toThrow();
      // testspec directory should still exist
      await expect(
        fs.access(path.join(testDir, "testspec")),
      ).resolves.not.toThrow();
    });

    it("should NOT delete testspec/project.md", async () => {
      const projectPath = path.join(testDir, "testspec", "project.md");
      ...
      expect(result.deletedFiles).not.toContain("testspec/project.md");
...
${TESTSPEC_MARKERS.end}`,
    ...
    it("should remove markers from root AGENTS.md even when only TestSpec content (never delete)", async () => {
      const agentsPath = path.join(testDir, "AGENTS.md");
      await fs.writeFile(
        agentsPath,
        `${TESTSPEC_MARKERS.start}\nTestSpec content\n${TESTSPEC_MARKERS.end}`,
  ...
  describe("formatCleanupSummary", () => {
    ...
    it("should format deleted directories", () => {
      const result = {
        deletedFiles: [],
        modifiedFiles: [],
        deletedDirs: [".claude/commands/testspec"],
        projectMdNeedsMigration: false,
        errors: [],
      };
      ...
      expect(summary).toContain(
        "✓ Removed .claude/commands/testspec/ (replaced by /testspec:*)",
      );
    });
    ...
    it("should format modified files", () => {
      ...
      const summary = formatCleanupSummary(result);
      expect(summary).toContain("  ✓ Removed TestSpec markers from AGENTS.md");
    });
    ...
    it("should include migration hint for project.md", () => {
      ...
      expect(summary).toContain("testspec/project.md");
  ...
  describe("formatDetectionSummary", () => {
    it("should include welcoming upgrade header and explanation", () => {
      ...
      expect(summary).toContain("Upgrading to the new TestSpec");
    ...
    it("should format slash command directories", () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [".claude/commands/testspec"],
        slashCommandFiles: [],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
      ...
      expect(summary).toContain("- .claude/commands/testspec/");
    });
    ...
    it("should format slash command files", () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [".cursor/commands/testspec-proposal.md"],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
      ...
      const summary = formatDetectionSummary(detection);
      expect(summary).toContain("Files to remove");
      expect(summary).toContain("- .cursor/commands/testspec-proposal.md");
    });

    it("should format testspec/AGENTS.md", () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
      ...
      const summary = formatDetectionSummary(detection);
      expect(summary).toContain("Files to remove");
      expect(summary).toContain("- testspec/AGENTS.md");
    });
    ...
    it("should group both removals and updates correctly", () => {
      ...
      expect(summary).toContain("Files to remove");
      expect(summary).toContain("Files to update");
      // Check removals (only slash commands and testspec/AGENTS.md)
      expect(summary).toContain(".claude/commands/testspec/");
      expect(summary).toContain("testspec/AGENTS.md");
  ...
  describe("formatProjectMdMigrationHint", () => {
    it("should return migration hint message", () => {
      ...
      expect(hint).toContain("testspec/project.md");
    ...
    it("should explain the new context section benefits", () => {
      const hint = formatProjectMdMigrationHint();
      expect(hint).toContain("included in every TestSpec request");
  ...
  describe("getToolsFromLegacyArtifacts", () => {
    ...
    it("should extract multiple tools from mixed legacy artifacts", () => {
      const detection = {
        ...
        slashCommandDirs: [
          ...
          ".qoder/commands/testspec",
        ],
        slashCommandFiles: [
          ".cursor/commands/testspec-apply.md",
          ".windsurf/workflows/testspec-archive.md",
        ],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
    ...
    it("should deduplicate tools when multiple files match same tool", () => {
      const detection = {
        ...
        slashCommandFiles: [
          ".cursor/commands/testspec-proposal.md",
          ".cursor/commands/testspec-apply.md",
          ".cursor/commands/testspec-archive.md",
        ],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
    ...
    it("should handle qwen TOML-based legacy files", () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [".qwen/commands/testspec-proposal.toml"],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it("should handle continue prompt files", () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [".continue/prompts/testspec-apply.prompt"],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it("should handle github-copilot prompt files", () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [".github/prompts/testspec-apply.prompt.md"],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it("should handle opencode testspec-* legacy files", () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [".opencode/command/testspec-propose.md"],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it("should handle opencode testspec-* legacy files", () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [".opencode/command/testspec-new.md"],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it("should deduplicate opencode when both testspec-* and testspec-* files exist", () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [
          ".opencode/command/testspec-propose.md",
          ".opencode/command/testspec-new.md",
        ],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
