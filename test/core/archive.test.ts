...
describe("ArchiveCommand", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `testspec-archive-test-${Date.now()}`);
    await fs.mkdir(tempDir, { recursive: true });

    // Change to temp directory
    process.chdir(tempDir);

    // Create TestSpec structure
    const testspecDir = path.join(tempDir, "testspec");
    await fs.mkdir(path.join(testspecDir, "changes"), { recursive: true });
    await fs.mkdir(path.join(testspecDir, "specs"), { recursive: true });
    await fs.mkdir(path.join(testspecDir, "changes", "archive"), {
      recursive: true,
    });
  ...
  describe("execute", () => {
    it("should archive a change successfully", async () => {
      ...
      const changeDir = path.join(tempDir, "testspec", "changes", changeName);
      ...
      const archiveDir = path.join(tempDir, "testspec", "changes", "archive");
      const archives = await fs.readdir(archiveDir);

      expect(archives.length).toBe(1);
      expect(archives[0]).toMatch(
...
Then expected result happens`;
    ...
    it("should support header trim-only normalization for matching", async () => {
      ...
      const mainSpecDir = path.join(tempDir, "testspec", "specs", "alpha");
...
Updated details.`;
    ...
    it("should apply operations in order: RENAMED → REMOVED → MODIFIED → ADDED", async () => {
      ...
      const mainSpecDir = path.join(tempDir, "testspec", "specs", "beta");
      await fs.mkdir(mainSpecDir, { recursive: true });
...
content D`;
    ...
    it("should abort with error when MODIFIED/REMOVED reference non-existent requirements", async () => {
      ...
      const mainSpecDir = path.join(tempDir, "testspec", "specs", "gamma");
      await fs.mkdir(mainSpecDir, { recursive: true });
...
- **THEN** qux changes`;
    ...
    it("should require MODIFIED to reference the NEW header when a rename exists (error format)", async () => {
      ...
      // Main spec with Old
      const mainSpecDir = path.join(tempDir, "testspec", "specs", "delta");
      await fs.mkdir(mainSpecDir, { recursive: true });
...
### Requirement: Missing`,
    ...
    it("should display aggregated totals across multiple specs", async () => {
      ...
      const psiMain = path.join(tempDir, "testspec", "specs", "psi", "spec.md");
  ...
  describe("error handling", () => {
    it("should throw error when testspec directory does not exist", async () => {
      // Remove testspec directory
      await fs.rm(path.join(tempDir, "testspec"), { recursive: true });

      await expect(
        archiveCommand.execute("any-change", { yes: true }),
      ).rejects.toThrow(
        "No TestSpec changes directory found. Run 'testspec init' first.",
      );
    });
  });
  ...
  describe("interactive mode", () => {
    it("should use select prompt for change selection", async () => {
      ...
      await fs.mkdir(path.join(tempDir, "testspec", "changes", change1), {
        recursive: true,
      });
      await fs.mkdir(path.join(tempDir, "testspec", "changes", change2), {
        recursive: true,
      });
