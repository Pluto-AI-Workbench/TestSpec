...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for TestSpec CLI');
			expect(script).toContain('$testspecCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for testspec command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize TestSpec',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName testspec');
			expect(script).toContain('-ScriptBlock $testspecCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-TestSpecChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-TestSpecSpecs');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-TestSpecChanges');
			expect(script).toContain('testspec __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-TestSpecSpecs');
			expect(script).toContain('testspec __complete specs 2>$null');
		});
