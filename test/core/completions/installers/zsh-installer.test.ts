...
describe('ZshInstaller', () => {
  ...
  beforeEach(async () => {
    ...
    testHomeDir = path.join(os.tmpdir(), `testspec-zsh-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return Oh My Zsh path when Oh My Zsh is installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.oh-my-zsh', 'custom', 'completions', '_testspec'));
    });
    ...
    it('should return standard Zsh path when Oh My Zsh is not installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.zsh', 'completions', '_testspec'));
    });
  });
  ...
  describe('install', () => {
    const testScript = '#compdef testspec\n_testspec() {\n  echo "test"\n}\n';
    ...
    it('should install to Oh My Zsh path when Oh My Zsh is present', async () => {
      ...
      expect(result.isOhMyZsh).toBe(true);
      expect(result.installedPath).toBe(path.join(ohMyZshPath, 'custom', 'completions', '_testspec'));
    ...
    it('should install to standard Zsh path when Oh My Zsh is not present', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.isOhMyZsh).toBe(false);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.zsh', 'completions', '_testspec'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.zsh', 'completions', '_testspec');
    ...
    it('should include fpath instructions for standard Zsh when auto-config is disabled', async () => {
      const originalEnv = process.env.TESTSPEC_NO_AUTO_CONFIG;
      process.env.TESTSPEC_NO_AUTO_CONFIG = '1';
      ...
      if (originalEnv === undefined) {
        delete process.env.TESTSPEC_NO_AUTO_CONFIG;
      } else {
        process.env.TESTSPEC_NO_AUTO_CONFIG = originalEnv;
      }
    });
    ...
    it('should handle paths with spaces in .zshrc config', async () => {
      ...
      const testHomeDirWithSpaces = path.join(os.tmpdir(), `testspec zsh test ${randomUUID()}`);
  ...
  describe('uninstall', () => {
    ...
    it('should remove from correct location for Oh My Zsh', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain(path.join('.oh-my-zsh', 'custom', 'completions', '_testspec'));
    });
  });
  ...
  describe('getInstallationInfo', () => {
    ...
    it('should return installation info when installed', async () => {
      ...
      expect(info.path).toContain('_testspec');
  ...
  describe('configureZshrc', () => {
    ...
    it('should create .zshrc with markers and config when file does not exist', async () => {
      ...
      expect(content).toContain('# TESTSPEC:START');
      expect(content).toContain('# TESTSPEC:END');
      expect(content).toContain('# TestSpec shell completions configuration');
    ...
    it('should prepend markers and config when .zshrc exists without markers', async () => {
      ...
      const configIndex = content.indexOf('# TESTSPEC:START');
    ...
    it('should preserve user content outside markers', async () => {
      ...
      expect(content).toContain(`fpath=("${completionsDir}" $fpath)`);
      expect(content).not.toContain('# Old TestSpec config');
    });

    it('should return false when TESTSPEC_NO_AUTO_CONFIG is set', async () => {
      const originalEnv = process.env.TESTSPEC_NO_AUTO_CONFIG;
      process.env.TESTSPEC_NO_AUTO_CONFIG = '1';
  ...
  describe('removeZshrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      const content = [
        ...
        '',
        '# TESTSPEC:START',
        '# TestSpec shell completions configuration',
      ...
      expect(newContent).not.toContain('# TESTSPEC:START');
      expect(newContent).not.toContain('# TESTSPEC:END');
      expect(newContent).not.toContain('TestSpec shell completions');
    ...
    it('should handle invalid marker placement gracefully', async () => {
      ...
      await fs.writeFile(zshrcPath, '# TESTSPEC:END\n# TESTSPEC:START\n');
    ...
    it('should return true when only one marker is present', async () => {
      const zshrcPath = path.join(testHomeDir, '.zshrc');
      await fs.writeFile(zshrcPath, '# TESTSPEC:START\nsome config\n');
  ...
  describe('uninstall with .zshrc cleanup', () => {
    ...
    it('should remove .zshrc config when uninstalling', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain('Removed TestSpec configuration from ~/.zshrc');

      // Verify .zshrc config was removed
      content = await fs.readFile(zshrcPath, 'utf-8');
      expect(content).not.toContain('# TESTSPEC:START');
    });
