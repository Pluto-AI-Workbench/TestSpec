...
describe('PowerShellInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `testspec-powershell-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return path relative to profile directory', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'powershell', 'TestSpecCompletion.ps1'));
    });

    it('should work with custom PROFILE environment variable', () => {
      process.env.PROFILE = path.join(testHomeDir, 'custom', 'profile.ps1');
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'custom', 'TestSpecCompletion.ps1'));
    });
    ...
    it('should return Windows path when on Windows platform', () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'Documents', 'PowerShell', 'TestSpecCompletion.ps1'));
    });
  });
  ...
  describe('configureProfile', () => {
    const mockScriptPath = '/path/to/TestSpecCompletion.ps1';

    // Note: TESTSPEC_NO_AUTO_CONFIG check is now handled in the install() method,
    ...
    it.skipIf(process.platform === 'win32')('should skip configuration when script line already exists', async () => {
      ...
      const initialContent = [
        '# TESTSPEC:START - TestSpec completion (managed block, do not edit manually)',
        `. "${mockScriptPath}"`,
        '# TESTSPEC:END',
        '',
        '# My custom config',
  ...
  describe('removeProfileConfig', () => {
    ...
    it('should remove content between markers', async () => {
      ...
      expect(content).not.toContain('# TESTSPEC:END');
      expect(content).not.toContain('# TestSpec completions');
  ...
  describe('install', () => {
    const mockCompletionScript = `# PowerShell completion script for TestSpec
$testspecCompleter = {
    param($wordToComplete, $commandAst, $cursorPosition)
    # Completion logic here
}
Register-ArgumentCompleter -CommandName testspec -ScriptBlock $testspecCompleter
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toContain('TestSpecCompletion.ps1');
    ...
    // Note: TESTSPEC_NO_AUTO_CONFIG support was removed from PowerShell installer
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `testspec powershell test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('testspec powershell test');
  ...
  describe('encoding preservation', () => {
    ...
    it('should preserve UTF-16 LE BOM when removing profile config', async () => {
      ...
      const textWithBlock = [
        '. "C:\\Code\\profile.ps1"',
        '# TESTSPEC:START',
        '. "/path/to/TestSpecCompletion.ps1"',
        '# TESTSPEC:END',
        '',
      ].join('\n');
      ...
      // Verify content: original line kept, TestSpec block removed
      const content = raw.subarray(2).toString('utf16le');
    ...
    it('should round-trip UTF-16 LE through install �?uninstall without corruption', async () => {
      ...
      // Install adds the TestSpec block
      ...
      // Uninstall removes the TestSpec block
...
`;
    ...
    it('should handle both script and config removal', async () => {
      ...
      expect(profileContent).toContain('# TESTSPEC:START');
      ...
      expect(profileContentAfter).not.toContain('# TESTSPEC:START');
    });
