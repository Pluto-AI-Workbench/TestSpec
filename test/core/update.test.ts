...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `testspec-test-${randomUUID()}`);
    await fs.mkdir(testDir, { recursive: true });

    // Create testspec directory
  ...
  describe('basic validation', () => {
    it('should throw error if testspec directory does not exist', async () => {
      // Remove testspec directory
      await fs.rm(path.join(testDir, 'testspec'), {
        recursive: true,
        force: true,
      });

      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No TestSpec directory found. Run 'testspec init' first."
      );
    });
...
`;
      ...
      expect(updatedSkill).toContain('name: testspec-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'testspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'testspec-explore', 'SKILL.md'),
        'old content'
      );
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'testspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'testspec-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'testspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'testspec-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: testspec-explore');
      ...
      expect(cursorSkill).toContain('name: testspec-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'testspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'testspec-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: testspec-<id>.toml
      const qwenCmd = path.join(
        testDir,
        '.qwen',
        'commands',
        'testspec-explore.toml'
      );
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'testspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'testspec-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'testspec-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'testspec-apply-change', 'SKILL.md'),
        'old'
      );
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'testspec-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'testspec-explore', 'SKILL.md');
  ...
  describe('legacy cleanup', () => {
    it('should detect and auto-cleanup legacy files with --force flag', async () => {
      ...
      // Create legacy CLAUDE.md with TestSpec markers
      const legacyContent = `${TESTSPEC_MARKERS.start}
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new TestSpec')
      );

      // Should show marker removal message (config files are never deleted, only have markers removed)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed TestSpec markers from CLAUDE.md')
      );
...
`;
    ...
    it('should cleanup legacy slash command directories with --force', async () => {
      ...
      const legacyCommandDir = path.join(testDir, '.claude', 'commands', 'testspec');
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed .claude/commands/testspec/')
      );
    ...
    it('should cleanup legacy testspec/AGENTS.md with --force', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'testspec-explore'), {
        recursive: true,
      ...
      // Create legacy testspec/AGENTS.md
      await fs.writeFile(
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed testspec/AGENTS.md')
      );
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new TestSpec')
      );
    ...
    it('should remove TestSpec marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'testspec-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + TestSpec markers)
...
`;
      ...
      expect(updatedContent).not.toContain(TESTSPEC_MARKERS.start);
      expect(updatedContent).not.toContain(TESTSPEC_MARKERS.end);
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/testspec:new')
      );
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, '.claude', 'commands', 'testspec', 'proposal.md'),
        'content'
      );
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'testspec-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'testspec-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'testspec-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'testspec-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'testspec-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'testspec-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'testspec-new-change', 'SKILL.md');
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'testspec-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'testspec-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'testspec-explore', 'SKILL.md'), 'old');
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'testspec-new-change', 'SKILL.md')
      )).toBe(true);
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'testspec-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'testspec-new-change', 'SKILL.md'), 'old');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'testspec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'testspec-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'testspec init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'testspec init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'testspec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'testspec-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'testspec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'testspec-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'testspec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'testspec-apply-change', 'SKILL.md'), 'content');
