...
describe('skill-generation', () => {
  describe('getSkillTemplates', () => {
    ...
    it('should include all expected skills', () => {
      ...
      expect(dirNames).toContain('testspec-explore');
      expect(dirNames).toContain('testspec-new-change');
      expect(dirNames).toContain('testspec-continue-change');
      expect(dirNames).toContain('testspec-apply-change');
      expect(dirNames).toContain('testspec-ff-change');
      expect(dirNames).toContain('testspec-sync-specs');
      expect(dirNames).toContain('testspec-archive-change');
      expect(dirNames).toContain('testspec-bulk-archive-change');
      expect(dirNames).toContain('testspec-verify-change');
      expect(dirNames).toContain('testspec-onboard');
      expect(dirNames).toContain('testspec-propose');
    });
    ...
    it('should return single template when filter has one workflow', () => {
      ...
      expect(filtered[0].dirName).toBe('testspec-propose');
    });
  });
  ...
  describe('generateSkillContent', () => {
    ...
    it('should use default values for optional fields', () => {
      ...
      expect(content).toContain('license: MIT');
      expect(content).toContain('compatibility: Requires testspec CLI.');
      expect(content).toContain('author: testspec');
    ...
    it('should apply transformInstructions callback when provided', () => {
      const template = {
        ...
        instructions: 'Use /testspec:new to start and /testspec:apply to implement.',
      };

      const transformer = (text: string) => text.replace(/\/testspec:/g, '/testspec-');
      const content = generateSkillContent(template, '0.23.0', transformer);

      expect(content).toContain('/testspec-new');
      expect(content).toContain('/testspec-apply');
      expect(content).not.toContain('/testspec:new');
      expect(content).not.toContain('/testspec:apply');
    });
    ...
    it('should not transform instructions when callback is undefined', () => {
      const template = {
        ...
        instructions: 'Use /testspec:new to start.',
      };
