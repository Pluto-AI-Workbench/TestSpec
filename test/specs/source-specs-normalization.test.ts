  ...
  it("enforces required sections and bans hidden requirements, placeholders, and delta headers", async () => {
    ...
    for (const file of files) {
      ...
      const rawRequirementCount =
        stripFencedCodeBlocksPreservingLines(content).match(
          REQUIREMENT_HEADER_PATTERN,
        )?.length ?? 0;

      expect(content, `${relativeFile} must include ## Purpose`).toMatch(
        /^## Purpose$/m,
      );
      expect(content, `${relativeFile} must include ## Requirements`).toMatch(
        /^## Requirements$/m,
      );
      expect(
        content,
