...
TestSpec provides three levels of customization:
...
The `TestSpec/config.yaml` file is the easiest way to customize TestSpec for your team. It lets you:
...
```bash
# Without config
TestSpec new change my-feature --schema spec-driven

# With config - schema is automatic
TestSpec new change my-feature
```
...
When TestSpec needs a schema, it checks in this order:

1. CLI flag: `--schema <name>`
2. Change metadata (`.TestSpec.yaml` in the change folder)
...
When project config isn't enough, create your own schema with a completely custom workflow. Custom schemas live in your project's `TestSpec/schemas/` directory and are version-controlled with your code.
...
This copies the entire `spec-driven` schema to `TestSpec/schemas/my-workflow/` where you can edit it freely.

**What you get:**

```text
TestSpec/schemas/my-workflow/
...
```yaml
# TestSpec/schemas/my-workflow/schema.yaml
name: my-workflow
version: 1
...
```bash
# Specify on command
TestSpec new change feature --schema my-workflow
...
Schema: my-workflow
Source: project
Path: /path/to/project/TestSpec/schemas/my-workflow
```

---

> **Note:** TestSpec also supports user-level schemas at `~/.local/share/TestSpec/schemas/` for sharing across projects, but project-level schemas in `TestSpec/schemas/` are recommended since they're version-controlled with your code.
...
```yaml
# TestSpec/schemas/rapid/schema.yaml
name: rapid
version: 1
...
```bash
TestSpec schema fork spec-driven with-review
```
